...
import com.sqli.planification.model.*;
...
public class PlanningService implements IPlanningService {
    ...
    public List<PlanningCollaborateurList> getPlanningCollaborateurs(Long idplanning) {
        Planning planning = planningService.getPlanning(idplanning);
        List<Affecter> affecters = affecterService.getAllAffectation();
        List<PlanningCollaborateur> planningCollaborateurs = new ArrayList<>();
        List<PlanningCollaborateurList> planningCollaborateurLists = new ArrayList<>();
        List<BigInteger> collaborateursIds = planningRepository.getIdCollaborateurByPlanning(idplanning);
        int cmpt = 0;

        for (BigInteger idCollaborateur : collaborateursIds) {
            List<Semaine> semaines = semaineService.getSemainesByMoisEtAnnee(planning.getMoisDebut().getMois(),
                    planning.getAnneeDebut().getAnnee(), planning.getMoisFin().getMois(),
                    planning.getAnneeFin().getAnnee());
            for (Semaine semaine2 : semaines) {
                cmpt = 0;
                for (Affecter affecter : affecters) {
                    //System.out.println(affecter.getProjet().getIdProjet()+" = "+planification.getProjet().getIdProjet()+" | "+affecter.getSemaine().getIdSemaine()+" = "+semaine2.getIdSemaine());
                    if (affecter.getCollaborateur().getIdCollaborateur() == idCollaborateur.longValue()
                            && affecter.getSemaine().getIdSemaine() == semaine2.getIdSemaine()) {
                        cmpt = cmpt + affecter.getNbrJour();
                        //System.out.println(affecter.getProjet().getTitreProjet() + " --> " + cmpt);
                    }
                ...
                planningCollaborateurs.add(new PlanningCollaborateur(semaine2, cmpt));
            }
            Collaborateur collaborateur = collaborateurService.getCollaborateur(idCollaborateur.longValue());
            planningCollaborateurLists.add(new PlanningCollaborateurList(collaborateur, planningCollaborateurs));
            planningCollaborateurs = new ArrayList<>();

        }
        return planningCollaborateurLists;
    }
}
